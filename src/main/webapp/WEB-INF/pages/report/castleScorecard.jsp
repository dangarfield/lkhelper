...
<template:page pageTitle="${pageTitle}">
	...
	<div class="bs-header castle-report" id="content">
		<div class="container">
			<h1>${reportModelCastleData.castleData.name}</h1>
			<c:choose>
				<c:when test="${reportModelCastleData.allianceData.id gt 0}"> <%-- Player part of an alliance --%>
					<p>Currently with ${reportModelCastleData.castleData.points} on the ${serverData.name} server,
					${reportModelCastleData.castleData.name} belongs to ${reportModelCastleData.playerData.name} who is part of the ${reportModelCastleData.allianceData.name} alliance and has ${reportModelCastleData.playerData.castleCount} castles</p>
				</c:when>
				<c:otherwise> <%-- Player not part of an alliance --%>
					<p>Currently with ${reportModelCastleData.castleData.points} on the ${serverData.name} server,
					${reportModelCastleData.castleData.name} belongs to ${reportModelCastleData.playerData.name} who is not part of an alliance and has ${reportModelCastleData.playerData.castleCount} castles</p>
				...
				<div class="carbonad">
					<div id="azcarbon">
						<span class="carbonad-imaged">
							<a href="">
								<img id="adzerk_ad" class="carbonad-image carbonad-img" src="/assets/images/hero/castle.png" width="130" height="100" border="0" alt="Castle Report"/>
							</a>
						</span>
						...
						<ul class="nav bs-sidenav">
							<li>
								...
								<ul class="nav">
									...
									<li><a href="#castle-report-historic-castle-name">Historic Castle Name</a></li>
									<li><a href="#castle-report-historic-player-name">Historic Players</a></li>
									<li><a href="#castle-report-historic-alliance-name">Historic Alliances</a></li>
			...
			<layout:column cssClass="col-md-9" role="main">
				...
				<div class="bs-docs-section">
					...
					<div class="highchart historicPoints"
						...
						data-subtitle="Castle: ${reportModelCastleData.castleData.name}"
						data-series-count="2"
					...
					<image:castleMapImage castleId="${reportModelCastleData.castleData.id}" serverId="${serverData.id}" base="true" />
					...
					<image:castleMapImage castleId="${reportModelCastleData.castleData.id}" serverId="${serverData.id}" playerId="${reportModelCastleData.playerData.id}" base="true" />
					...
					<image:castleMapImage castleId="${reportModelCastleData.castleData.id}" serverId="${serverData.id}" playerId="${reportModelCastleData.playerData.id}" allianceId="${reportModelCastleData.allianceData.id}" base="true" />
